...
const PlayerInfo = ({ player, playerIcon, mt }) => {
  ...
  return (
    ...
    <div className={`${styles.flexCenter} ${mt ? 'mt-2' : 'mb-4'}`}>

        <img 
         data-for={`Player-${mt ? '1' : '2'}`} 
         data-tip
         src={playerIcon} 
         alt={`player${mt ? '1' : '2'} Icon`} 
        ...
        >
            {//spread out all available health points(25) along with each's key, then map each item to each of the rendered self-closing divs
              ...
              )) //the functions healthLevel & marginIndexing both control div coloring and margins (ie for the individual health bars)
            //this creates the healthbar with 25 divs representing each health points and their colors change as number of points reduce
            }
